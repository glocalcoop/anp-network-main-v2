...
if ( function_exists( 'bp_post_get_permalink' ) ) { // ugly ugly ugly hack to check for pre-1.2 versions of BP
		...
		<?php do_action( 'bp_before_group_send_invites_content' ) ?>
			...
			<?php endif; ?>
				...
				<div class="left-menu">
					...
					<h3><?php _e("Search for Members to Invite", 'invite-anyone') ?></h3>
					...
					<h3><?php _e( 'Select Members to Invite', 'invite-anyone' ) ?> </h3>
				...
				<div class="main-column">
					...
					<div id="message" class="info">
						<p><?php _e('Invite Members by Searching or Selecting from List', 'invite-anyone'); ?></p>
					</div>
			...
			<?php if ( $event != 'create' ) : ?>
				<div class="clearfix"><input type="submit" name="submit" id="submit" value="<?php _e( 'Send Invites', 'invite-anyone' ) ?>" /></div>
	...
	<div class="left-menu">
		...
		<?php if ( ! invite_anyone_is_large_network( 'users' ) ) : ?>
			<p><?php _e( 'Select Members to Invite', 'invite-anyone' ) ?></p>
		...
		<?php if ( bp_group_has_invites() ) : ?>
			...
			<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
				...
				<li id="<?php bp_group_invite_item_id() ?>">
					...
					<div class="item">
						<div class="item-title"><?php bp_group_invite_user_link() ?>
						</div>
						<div class="item-meta">
							<span class="activity"><?php bp_group_invite_user_last_active() ?></span>
						</div>
